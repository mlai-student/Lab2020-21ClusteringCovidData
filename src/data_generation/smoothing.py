...
def smooth_timeline(X, Y, group_sort, start, end, data_gen_config):
    ...
    for i in range(start, end):
        X_out.append(round(float(group_sort.iloc[i-nr_days_for_avg+1: i+1].to_numpy().dot(conv_matrix))))
    Y_out = round(float(group_sort.iloc[end+1-nr_days_for_avg+1: end+1+1].to_numpy().dot(conv_matrix)))
