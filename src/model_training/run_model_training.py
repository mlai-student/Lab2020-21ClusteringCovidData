...
def run_model_training_main(train_config):
    ...
    knn = kNeighbors(X_train, 5)
    save_time_series_model(knn, [X_train, X_test, y_train, y_test])
...
def save_time_series_model(model, examples):
    try:
        ...
        pkl_file = open("data/{}/{}/{}_{}".format(today, "model", str(model.__class__.__name__), "data"), "wb")
